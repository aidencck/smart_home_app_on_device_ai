...
- **核心逻辑**:
  ...
  - **数据脱敏与聚合**: 戒指自身或 Edge Hub 在 5 分钟滑动窗口内计算出 `sleep_stage` (如 AWAKE, LIGHT\_SLEEP, DEEP\_SLEEP)。
...
- **核心逻辑**:
  - **双向数据流**: 接受控制指令并上报当前状态 (angle, vibration, support\_level, temp\_mode)。
